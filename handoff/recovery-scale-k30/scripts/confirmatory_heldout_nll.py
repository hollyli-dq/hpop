...
# K and m are read from the chains, not from the frozen K=3 / m=5 constants. Importing
# them made this script silently wrong at any other model size: the state would be built
# over three skills while the draws carried a different number.
N_CHAINS = 4
...
def dimensions_from_chain(path: Path) -> tuple:
    """(K, m) from a chain's `u_draws` of shape (draws, K, m, d)."""
    with np.load(path, allow_pickle=True) as payload:
        shape = np.asarray(payload["u_draws"]).shape
    if len(shape) != 4:
        raise ValueError(f"u_draws must be (draws, K, m, d), got {shape}")
    return int(shape[1]), int(shape[2])


def legal_tiling(length: int, index: int, n_skills: int) -> Segmentation:
...
def build_model(traces, n_skills: int, n_roles: int) -> Stage6EModel:
...
def make_state(model, fixed, u, pi, p) -> Stage6EState:
    return Stage6EState(
        segmentations=tuple(legal_tiling(len(t), n, int(model.n_skills))
...
def main() -> None:
    ...
    traces = load_heldout()
    n_skills, n_roles = dimensions_from_chain(RUN / "chains" / "full_cond_0.npz")
    model = build_model(traces, n_skills, n_roles)
    print(f"model read from the chains: K={n_skills}, m={n_roles}", flush=True)
    ...
    report = {
        ...
        "model": {"n_skills": int(model.n_skills), "n_roles": int(model.n_roles),
                  "min_width": MIN_BLOCK_WIDTH, "max_width": MAX_BLOCK_WIDTH,
