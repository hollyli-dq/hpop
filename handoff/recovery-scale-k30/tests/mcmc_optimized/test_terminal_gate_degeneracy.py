...
# `library_ids` now takes the number of skills explicitly. It used to assume three, which
# reshaped silently and wrongly at any other K -- these tests passed a width of 60 and got
# K = 3 by luck of the arithmetic. Passing it makes what they are testing exact.
K_HERE = 3


def test_library_ids_are_invariant_to_skill_relabelling():
    rng = np.random.default_rng(7)
    base = rng.random((10, K_HERE, 20)) < 0.3
    permuted = base[:, [2, 0, 1], :]
    a, _ = gate.library_ids(base.reshape(10, 60), K_HERE)
    b, _ = gate.library_ids(permuted.reshape(10, 60), K_HERE)
...
def test_library_ids_separate_genuinely_different_structures():
    ...
    draws[0, 0] = True
    draws[1, 1] = True
    ids, _ = gate.library_ids(draws, K_HERE)
    assert ids[0] != ids[1]


def test_library_ids_requires_the_skill_count_rather_than_assuming_three():
    """The regression this signature change exists to prevent."""
    draws = np.zeros((2, 60), dtype=bool)
    with pytest.raises(TypeError):
        gate.library_ids(draws)                    # no silent K = 3 fallback
    with pytest.raises(ValueError, match="not divisible"):
        gate.library_ids(draws, 7)                 # 60 % 7 != 0 -> loud, not a reshape
